...
**Updated 2026-08-28 (Alex):** a **paid C1/C2 band** is added on top of this model — see **§5**. Logged only; not scoped, not being built, and it lands *after* everything in §7.
...
**Refined 2026-08-28 (Alex) — the one exception to "no feature is behind the wall":** a **C1/C2 band** above the current ceiling, AI-heavy and paid. The line becomes two things rather than one dial: **free = the complete ladder every language targets today (through B2) + the checkpoints; paid = on-demand companion volume *plus* the C1/C2 band.** Nothing a free learner has today moves behind the wall — the paid side is *more ladder*, sold to someone who already climbed the free one. Full design in §5.
...
- The checkpoint is **participation, not perfection** — it does *not* block progression. Completing the unit doesn't require "passing" a conversation. (Open decision 6.4.)
...
## 5. C1/C2 — the paid, AI-native band (Alex, 2026-08-28)

**Status: logged, not scoped, not being built.** Alex's addition: **C1/C2 content that uses the AI heavily, and users pay for it.** It sits on top of everything above and is the last thing built (§7, step 5).

**Why "AI-heavy" and "paid" are the same decision — the non-obvious part.** All curriculum ships **in the client bundle**: `src/data/index.js` statically imports every language barrel into `UNITS`, and nothing fetches content at runtime. So *static* C1/C2 units cannot be gated — anyone can read them out of the bundle (and the repo is public today besides). Content-gating would mean serving units from the server: a new delivery path, a new persistence story, its own project. **A band whose value is live conversation has no such problem** — the expensive, un-copyable part is the ConvAI session, minted server-side at `api/convai-session.js`, which is exactly where §4's JWT + tier + usage check already has to go. So building C1/C2 *as* companion work is not only pedagogy; it is what makes the band gateable at all with the architecture we have. Design it deliberately that way: **thin authored scaffolding, thick companion.**

**It is also the right shape at that level.** C1/C2 is register, nuance, idiom, argument, holding a long turn — not more vocab rows. That is the companion's job, and §2's two-surface split already serves it: a C1 "lesson" is plausibly a checkpoint *with a syllabus*, rather than drill cards followed by a checkpoint.

**Contract work — its own scoped PR (content/engine separation stands).** C1/C2 exist nowhere in the code today: `CEFR_ORDER` in `src/data/contract.js` is `{A1,A2,B1,B2}` and hard-fails any other lesson `cefr` **or** language `target`; `VALID_STAGE` is `["pre-a1","a1","a2","b1","b2"]` and drives the Ladder's section grouping, so a C1 section is new UI; `CONTENT.md` documents A1–B2; `LANGUAGES.md` states every language targets B2 (all 22 entries); `milestones.js` builds one level milestone per band per language, so C1/C2 need new ids (`level-C1`, `level-C2-fr`…) — and the French lesson applies: adding a band must not inflate an existing band's denominator and silently un-earn it for mid-progress learners.

**Cost is the tier's price floor — cost it before authoring.** Drill cards cost ~0 to serve; a band that is mostly live conversation bills per minute per learner, every time. Before one C1 unit is authored, answer: **how many companion minutes does a C1 lesson assume, and what does that make the monthly serving cost of an active C1 learner?** That number sets the tier price — not competitor pricing. Same unresolved unit as the free-dial cap in §4.

**Open questions (log only, no decision yet):**
1. **Per-language or account-wide?** Finish fr B2 → buy fr C1, or one purchase opens C1/C2 everywhere.
2. **Own SKU, or the top tier of the volume dial?** Simplest is the top tier including it — one ladder of tiers instead of two products.
3. **What is a C1 "unit"?** Does it still emit items/SRS rows (so the ladder, milestones and review engine extend unchanged), or is it conversation + a review list (a fork in the machinery)? This decides most of the engine scope.
4. **Comped users** (§7 step 2's manual `profiles.tier` flag) presumably get it too — confirm.

**Sequencing.** Unchanged from Alex's park: fr/es/ja finish their current bands → companion v1 proves the mechanic (§7 steps 1–3) → **billing has to be real (step 4) before a paid band means anything** → C1/C2.
...
4. **Billing** — Stripe → webhook → `profiles.tier`. Its own brief.
5. **The C1/C2 band** (§5) — last. It is the first thing that *requires* step 4 to be real, and it starts with the contract change + a costed lesson shape, not with authored units.
