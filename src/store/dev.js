...
import { shouldListen, shouldCloze, isTraceable } from "./cardRouting.js";
...
function pickForKind(seed, kind) {
  ...
  switch (kind) {
    ...
    case "cloze:choice":  return { item: vocab.find((it) => shouldCloze(it)), rung: 2 };
    case "type:meaning":  return { item: vocab.find((it) => !shouldCloze(it)) ?? vocab[0], rung: 2 };
