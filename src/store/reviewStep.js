...
import {
  isLatin, earCrowdedOut, isTraceable, shouldListen, shouldReverseChoice, shouldListenType, shouldTypeReading, shouldTypeProduce, shouldSpeak, shouldCloze, shouldParticleCloze, shouldSentence, shouldConjugate, canBuildReading , hasAudio, produceIsFreePass, meaningIsFreePass } from "./cardRouting.js";
...
export function reviewStepFor(item) {
  ...
  if (rung === 2) {
    ...
    // The meaning card is the rung-2 fallback, so a free-pass item would land here
    // and grade correct for typing the prompt back. Send it somewhere that still
    // tests recall: dictation if it owns a clip (hearing it is never a free pass),
    // otherwise the 4-option reverse, where the distractors do the work.
  ...
  if (rung === 3) {
    ...
    // pronunciation is exactly what differs between "important" and "important".
