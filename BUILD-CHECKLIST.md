...
**Roles live in `CLAUDE.md` → "Roles — read this first."** Three CC lanes: **Feature CC** (app/engine/UX, draft PRs), **Curriculum CC** (content units, self-merges on green), **QA CC** (tests + reports, builds nothing, never merges). Alex merges everything. Two report/route sections in this doc: **Feature CC backlog** (engine/app work surfaced by other lanes) and **QA findings** (bugs/defects surfaced by QA, routed to a lane).
