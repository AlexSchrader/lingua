...
**Brief status:** ✅ **R1 cloze** — `BUILD-BRIEF-cloze-card.md` written (2026-07-04), ready to build. ⏳ **R7 mascot cast** — brief needed before code (design + asset spec). ⏳ **R8 Haruki "why?"** — brief needed before code (graded-bank boundary).
...
- [ ] **[R1] (Feature CC) Cloze card: fill the word into its own example sentence. → full brief written: `BUILD-BRIEF-cloze-card.md` (Idea CC, 2026-07-04).** *Biggest lever on "understanding over memorization" we're missing.* Every card tests an item in isolation; comprehension is seeing a word work *inside* a sentence. Show the item's `example.jp` with the target blanked (`＿＿をたべます`) + the `example.en` gloss as the clue; learner picks (v1 `cloze:choice`) or types (`cloze:type`, later) the missing JA word. Interleaves vocab, drills particles, makes grammar *do work*. **Data already exists** (every vocab/kanji item has `example {jp,en}`, validated). New card kind → `LIVE_CARD_KINDS` + coverage fixture + a `canCloze` routing guard. Reuses `buildOptions("front")` + `deriveGrade` + the listening card's `cardRouting.js` pattern; **no rung/FSRS/mastery/schema change** (pure variant). Brief has ground-truth file refs, the prompt design, edge cases, open decisions, and build phases. **Ready to build on Alex's go.**
...
- [ ] **[R6] (Feature CC, small Curriculum assist) Furigana toggle for kanji & sentences.** As kanji + grammar sentences arrive, beginners need reading scaffolding they can switch off as they grow — direct parallel to the existing Show-romaji toggle. Render furigana over kanji in examples/cards; data is the item `reading` (may need a small per-example reading assist from Curriculum for multi-kanji sentences). Scaffold that fades, not a crutch that stays.
