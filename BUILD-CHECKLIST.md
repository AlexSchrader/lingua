...
- [~] **[R35 = M.0 + M.1a] (Feature CC) Milestones — capability achievements. STARTED 2026-07-10 (feat/milestones, off main). → `BUILD-BRIEF-milestones.md` (decision doc on PR #70).** The positive replacement for R17/R18/R30. **Shipped (additive, no user-state change):** **M.0** — `src/data/milestones.js`: a content-agnostic milestone catalog (script "can read all hiragana/katakana/yōon", kanji first→all, vocab 10/50/100/250 *mastered*, per-CEFR-band "level complete" derived from `UNITS`) + pure `earnedMilestones(items)` / `nextMilestone(items)` / `milestoneSummary(items)` selectors, reusing `isMastered`/`isReviewable`; 7 unit tests (`tests/unit/milestones.test.mjs`). **M.1a** — a **Milestones** section at the top of Stats: earned list (never a wall of locked badges) + the single nearest next goal with a gentle progress bar; pure-derived, + smoke test. **Guardrail held:** every milestone rewards *capability*, never *activity*. **Deferred to Alex's check-in (touch persisted state):** **M.1b** — earned-once persistence (`milestonesEarned` set) + fire the dormant mascot `"achievement"` reaction on a *new* unlock; **M.2** — remove/retire the Today streak/XP/freezes trio (migration-safe). Both wait for the go-ahead.
