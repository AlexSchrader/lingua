...
- **⚡️ Next curriculum (lint now guards the mechanical half):** **Units 11–12** — A1 thematic vocab (e.g. colors/weather, transport/places) — then **first kanji** (needs a new `kanji` item type → contract change + brief) and particles/grammar; then A2. Plus the Unit 4–10 native/Claude review + the autonomous-authoring decision (Phase 2).
- **Decisions waiting on Alex:** (1) **autonomous curriculum merging** (curriculum-lint brief Part 3 — let CC merge green units without per-unit approval; overrides "draft PRs only"); (2) **add the `kanji` item type** (scoped contract change); (3) **accounts in production** — add `VITE_SUPABASE_*` env to Vercel + redeploy + add the prod URL to Supabase redirect allow-list (Apple sign-in later, needs Apple Dev account).
