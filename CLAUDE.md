...
- **Feature CC** — builds the app: the learning engine, screens, store, components, card kinds, theme, and the schema/contract. Day to day:
  ...
  - **Draft PR → Alex merges** (engine/schema keep a human gate — they can break everything). Update `BUILD-CHECKLIST.md` as part of the work. *(Deploy is normal: push `main` → Vercel auto-deploys production; `main` is the source of truth for prod. GitHub access is fine — the 2026-06-30 suspension was lifted 2026-07-04.)*
