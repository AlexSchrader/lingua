...
**Lesson shape, settled by Alex 2026-09-13:** every special letter the language has as its own glyph card, **plus a minimal exemplar word** so the lesson is not a bare alphabet (`ø` as a glyph card, then `ei øy`). **A short lesson is fine** — Norwegian has four special letters and does not invent two more, and the exemplar word is what fills it with real content. (The engine does give every item three screens plus spaced review, so the floor is a real lesson — but **that is not the justification**: the 3× applies to every lesson equally and cancels out, so a short lesson is genuinely short. The shape is the reason, not the multiplier.) ⚠️ **Never repeat an id to pad the count** — duplicate ids are a hard validator error. The principle that decides close calls: *unit 1 gets the learner pronouncing the letters before they are handed a word.* Full shape + the per-language inventory in `RUNBOOK-new-language.md` §4.
