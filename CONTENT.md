...
**Lesson shape, settled by Alex 2026-09-13:** every special letter the language has as its own glyph card, **plus a minimal exemplar word** so the lesson is not a bare alphabet (`ø` as a glyph card, then `ei øy`). **A short lesson is fine** — Norwegian has four special letters and does not invent two more. The engine already gives every item **three screens** (teach + two checks) plus spaced review, so a four-letter lesson is twelve screens; the density rule counts distinct items, not screens. ⚠️ **Never repeat an id to pad the count** — duplicate ids are a hard validator error. The principle that decides close calls: *unit 1 gets the learner pronouncing the letters before they are handed a word.* Full shape + the per-language inventory in `RUNBOOK-new-language.md` §4.
