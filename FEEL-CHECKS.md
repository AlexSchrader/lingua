...
| 9 | ⚠️ **Is the first Spanish lesson too long now?** Adding the five accented vowels takes `es-u1l1` from 7 items to **12** — and teaches run before any check, so that is **12 consecutive teach screens, then 24 checks: 36 steps**, against 24 in every other lesson of the unit. The first five are silent, visually near-identical accented vowels back to back. | Dev Mode → Spanish → unit 1, lesson 1 | Two gates looked at it and split: the lint band (5–8) says too long, the content gate says keep it and that splitting is strictly worse (the vowels fit no other lesson, and a new lesson renumbers l2–l4 → id changes → mastery wipe). It is an overwhelm judgement, which is yours. | es u1 glyph pass, 2026-09-13 |
| 10 | **`ú` and `ü` are the same card on three of four kinds.** Both fold to reading `"u"`, so `choice` ("which sound is this?") and `listen:type` cannot separate them — only `type:produce`, where you have to find the key, can. Does meeting them in different lessons (l1 and l2) make that read as two letters, or as one letter shown twice? | Dev Mode → Spanish → unit 1, lessons 1 and 2 | The fold is an answer key by design, not a pronunciation guide, so no validator can call this wrong — it is only a question of how it feels to learn. | es u1 glyph pass, 2026-09-13 |
