...
5. **`src/data/<lang>/TAUGHT-WORDS.md` — every word your language already teaches, and the slot that owns it.** Generated onto the scaffold branch before you existed, so it is already in your worktree. **Grep it, don't read it.** It answers the two questions that cost a whole lesson to get wrong: *may I use this word in an example?* (is it there, at a slot at or before mine) and *may I teach this front?* (if it is there, no — the lower slot owns it, §4). It also lists which slots each block owns, so you can tell when a word belongs to a block that has not written it yet. It cannot see what your siblings are writing right now; nothing can.
6. **`src/data/fr/unit1.js` — Latin-script *conventions* only** (the `front`/`reading` split, articles-with-nouns, accent folding). Its header comment is authoritative for how a Latin language encodes items; ja stays authoritative for how a language is *structured*.
...
- **Every vocab item gets a `drill` as you write it — not in a later pass.** `drill` is a SECOND short sentence beside `example`, same `{ jp, en }` shape (`jp` holds the target language; the key name is historical).
  ...
  - ⚠ **In an inflecting language the front often cannot appear in a natural short sentence at all, and `lint:curriculum` will not tell you** — lint only checks that the drill CONTAINS the front as a substring, while `tests/unit/drill-corpus.test.mjs` checks that the ROUTER can find it as a whole word. German B2 block 2 hit all three shapes of this, 12 cards' worth, every one green in lint and red in the test:
    - **A split verb bracket.** *hätte … gehabt* and *sieht … vor* are never adjacent, so a separable verb or a compound Konjunktiv can only be a front if the drill uses a modal (*will eine Prüfung vornehmen*) or the cluster stays contiguous (*hätte sein können*).
    - **A preposition that leads its noun.** *beruhen auf* and *Wert legen auf* never occur as written, because the auf-phrase comes first. Teach the bare verb and put the preposition in the `hint`.
    - **An attributive adjective.** *der vermeintliche Fehler* does not contain *vermeintlich* as a word. Use the predicative or adverbial form in the drill — and note that an attributive-ONLY adjective (German *jeweilig*) can never be a card, so pick a different word.
...
- **Word ownership: lower slot number wins.** If your block wants a word an earlier block already teaches, you don't teach it — you use it in examples. If a *later* block wants one of yours, it defers to you. Never coordinate with the other sessions; the rule decides.
  ...
  - 🛑 **"LEXEME" MEANS INFLECTION, NOT DERIVATION — AND THAT DISTINCTION HAS BEEN COSTING REAL WORDS.** Read the bullet above literally: it says *inflectional relatives* — the SAME word in another form (`見る`/`見ます`, an infinitive vs its conjugation, `ei fare`/`en fare`). **A base word and a word DERIVED from it are two different lexemes and both may be taught.** `fragen` (verb) and `die Frage` (noun) are no more one lexeme than `das Buch` and `buchen`. ⚠️ **Seats have been reading this rule as a ban on derivation, and it has blocked some of the commonest words in the language.** Measured on `main` 2026-09-23: German has **17 core words with no card** — `die Frage`, `die Antwort`, `wahr`, `der Punkt`, `offen`, `der Teil`, `der Plan`, `lösen`, `anders`, `bedeuten`, `die Hoffnung`, `der Mut`, `das Glück` among them — each one withheld because the course already owned a DERIVATIVE (`die Umfrage`, `die Lösung`, `die Wahrheit`, `pünktlich`). The course teaches *survey*, *enquiry* and *demand* but not **question**. Meanwhile the same corpus already carries **344 base+derivative pairs**, 205 of them entirely pre-B2, including `die Zeit` and `die Zeitung` **in the same unit 1**. The rule was never enforced — it was only ever applied to the words a seat happened to doubt. **So: card the base word.** The test is not "do these share a stem" but **"would a learner who knows one already know the other?"** — yes for `fragen`→`fragte`, no for `fragen`→`die Frage`. Norwegian B2 block 1 cut 10 fronts on this reading; re-examine them before accepting the cut.
- **A GLOSS IS A PROMPT — two words may not share one.** The produce card shows `meaning` and accepts only that item's own `front`, so two items glossed "of course" are one card with two right answers, and a learner typing the synonym YOUR OWN lesson taught is marked wrong. Fronts are guarded (above); glosses were not, until `lint:curriculum` started warning on 2026-09-16 — **628 collisions were already in the corpus when the check landed**. Give each a discriminator, the way `es-u14l4-todo` is "all (masculine)" rather than "all". One word in two scripts (same gloss AND same reading, like さかな/魚) is fine and does not warn.
...
3. **Review what comes back.** Run the checks no single block can run on itself —
   duplicate fronts, lexeme duplicates — **the SAME word in another form** (infinitive vs conjugated, article variants); **NOT a noun and a verb derived from it, which are two lexemes and may both be taught**, meaning-field overlaps, reading collisions, same-lesson pairs. **Fix what
...
7. **Regenerate the taught-words list: `npm run taught:words -- <lang>`, and commit it.** Content is final at this point, so this is when it becomes true again — and it is what the NEXT band's crews will author against. Skipping it hands them a list a whole band out of date, which is the hole this file exists to close.
8. `git worktree remove` the three authoring worktrees.
